...
class SwarmbotAgent(base_agent.BaseAgent):
	...
	def build_order(unit, attachment=None, move_number):
		if  move_number == 0: ## select worker
			self.point_selected = None ## reset point selected since we are about to select something
			if attachment is None: ## we are not doing techlab or reactor 
				unit_y, unit_x = (self.unit_types == _SCV).nonzero()
				if unit_y.any():
					i = random.randint(0, len(unit_y) - 1)
					target = [unit_x[i], unit_y[i]]
					self.point_selected = target
				...
				if unit_y.any():
					...
					target = [unit_x[i], unit_y[i]]
					self.point_selected = target
					## might be selecting more than one building at some point. 
					return (_SELECT_POINT, [_NOT_QUEUED, target])
		elif move_number == 1: ## build building 
			if attachment is None:
				if self.unit_dict[unit] in obs.observation['available_actions']:
					unit_y, unit_x = (self.unit_types == self.unit_dict[unit]).nonzero()
					## might have to check if currently there is ntohing similar built in the same place. 

			if self.unit_dict[unit] in obs.observation['available_actions']:
				if self.unit_counts[self.unit_dict[unit]] > 0:
					unit_y, unit_x = (self.unit_types == self.unit_dict[attachment]).nonzero()
					## might need to check if its selected at this step. 
					if self.point_selected is not None:
						return actions.FunctionCall(self.smart_actions[], [_NOT_QUEUED, self.point_selected])
	...
	def step(self, obs):
		...
		if self.move_number == 0:
			...
			if action == 'b':
				## returns an action id and the parameters for that action id
