...
class SwarmbotAgent(base_agent.BaseAgent):
    def __init__(self):
        ...
        # actions that were returned to the base agent
        self.actions_taken = [(-1, [-1, -1])]
    ...
    def findLocationForBuilding(self, size, distance=6, chance=10):
        ...
        if mf_y.any():
            while True:
                ...
                s_target_x = int(
                    self.depot_x + np.random.choice([-1, 0, 1], 1) * distance)
                s_target_y = int(
                    self.depot_y + np.random.choice([-1, 0, 1], 1) * distance)
                # if the point fits in the screen
                within_map = (0 < s_target_x < max_x -
                              size) and (0 < s_target_y < max_y - size)
                # checks to see if there is space available for the building in between other buildings
                area = self.unit_types[s_target_y: s_target_y +
                                       6][s_target_x: s_target_x + 6]
                ...
                within_mineral_field = (min(mf_y) < s_target_y < max(mf_y)) and (
                    min(mf_x) < s_target_x < max(mf_x) + 11)
                chance += -1
    ...
    def update_counts(self, obs):
        ...
        if _SUPPLY_DEPOT in self.unit_counts:
            if unit_y.any():
                self.unit_counts[_SUPPLY_DEPOT] = int(
                    math.ceil(len(unit_y) / building_sizes['supplydepot']))
        else:
        ...
        if _REFINERY in self.unit_counts:
            if unit_y.any():
                self.unit_counts[_REFINERY] = int(
                    math.ceil(len(unit_y) / building_sizes['refinery']))
            else:
        ...
        if _BARRACKS in self.unit_counts:
            if unit_y.any():
                self.unit_counts[_BARRACKS] = int(
                    math.ceil(len(unit_y) / building_sizes['barracks']))
            else:
    ...
    def get_current_state(self, obs, hot_squares):
        ...
        # since we dont need player id or warpgate and larva counts
        player_info = player_info[1:9]
        ...
        #	else:
        #		temp.append(0)
        reversed_actions_taken = [self.actions_taken[i * -1][0]
                                  for i in range(1, len(self.actions_taken))]
        if len(reversed_actions_taken) < 10:
            reversed_actions_taken = [
                0] * (10 - len(reversed_actions_taken)) + reversed_actions_taken
        else:
    ...
    def step(self, obs):
        ...
        if obs.last():
            ...
            self.qlearn.learn(str(self.prev_state),
                              self.prev_action, self.reward, 'terminal')
            self.reset()
            self.prev_action = None
            self.prev_state = None
            self.move_number = 0
        ...
        if self.move_number == 0:
            ...
            if self.prev_action is not None:
                rr = self.getReward()
                self.qlearn.learn(str(self.prev_state),
                                  self.prev_action, rr, str(self.current_state))
            self.prev_assign()
...
## ------------------------------------------------------------------------------------------------------------------- ##
            ...
            if action == 'b' and unit == 'supplydepot' and _SELECT_POINT in obs.observation['available_actions']:
                ...
                if unit_y.any():
                    ...
                    target = temp if temp is not None else [
                        unit_x[j], unit_y[j]]
                    self.point_selected = (target, 'scv')
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                ...
                if unit_y.any():
                    ...
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                    self.actions_taken.append(
                        (_SELECT_POINT, [_SELECT_ALL, target]))
...
## ------------------------------------------------------------------------------------------------------------------- ##
        ...
        if self.move_number == 1:
            ...
            if action == 'b' and unit == 'supplydepot' and _BUILD_SUPPLY_DEPOT in obs.observation['available_actions']:
                target = self.findLocationForBuilding(
                    building_sizes['supplydepot'])
                ...
                self.unit_types = obs.observation['screen'][_UNIT_TYPE]
                unit_y, unit_x = (self.unit_types ==
                                  _NEUTRAL_VESPENE_GEYSER).nonzero()
                if unit_y.any():
                    i = random.randint(0, len(unit_y) - 1)
                    target = [unit_x[i], unit_y[i]]
                    self.point_selected = None
                    if _REFINERY in self.unit_counts and self.unit_counts[_REFINERY] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_REFINERY, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'scv' and target is not None:
                    ...
                    if _FACTORY in self.unit_counts and self.unit_counts[_FACTORY] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_FACTORY, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'scv' and target is not None:
                    ...
                    if _STARPORT in self.unit_counts and self.unit_counts[_STARPORT] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_STARPORT, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'scv' and target is not None:
                    ...
                    if _ARMORY in self.unit_counts and self.unit_counts[_FACTORY] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_ARMORY, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'scv' and target is not None:
                    ...
                    if _FUSION_CORE in self.unit_counts and self.unit_counts[_FUSION_CORE] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_FUSION_CORE, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'scv' and target is not None:
                    ...
                    if _GHOST in self.unit_counts and self.unit_counts[_GHOST] <= 2:
                        ...
                        self.states_happened.append(
                            (self.prev_state, self.move_number))
                        self.actions_taken.append(
                            (_BUILD_GHOST_ACADEMY, [_NOT_QUEUED, target]))
                ...
                if obs.observation['player'][6] < obs.observation['player'][4]:
                ...
                if do_it and _ATTACK_MINIMAP in obs.observation["available_actions"]:
                    ...
                    target = self.transformLocation(
                        int(x) + (x_offset * 8), int(y) + (y_offset * 8))

                    self.point_selected = None
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                    self.actions_taken.append(
                        (_ATTACK_MINIMAP, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'barracks':
                    ...
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                    self.actions_taken.append(
                        (_BUILD_TECHLAB, [_NOT_QUEUED, target]))
                ...
                if self.point_selected is not None and self.point_selected[1] == 'barracks':
                    ...
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                    self.actions_taken.append(
                        (_BUILD_REACTOR, [_NOT_QUEUED, target]))
...
## ------------------------------------------------------------------------------------------------------------------- ##
        ...
        if self.move_number == 2:
            ...
            if action == 'b' and unit != 'refinery' and _HARVEST_GATHER in obs.observation['available_actions']:
                self.unit_types = obs.observation['screen'][_UNIT_TYPE]
                unit_y, unit_x = (self.unit_types ==
                                  _NEUTRAL_MINERAL_FIELD).nonzero()
                if unit_y.any():
                    i = random.randint(0, len(unit_y) - 1)
                    ...
                    self.states_happened.append(
                        (self.prev_state, self.move_number))
                    self.actions_taken.append(
                        (_HARVEST_GATHER, [_NOT_QUEUED, target]))
